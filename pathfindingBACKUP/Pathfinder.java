...
class Pathfinder {
	 ...
	 */
	public Pathfinder(TileBasedMap map, int maxSearchDistance, boolean allowDiagMovement) {
		this.map = map;
	...
	public Path findPath(Mover mover, int sx, int sy, int tx, int ty) {
		...
		Node closestNode = nodes[sx][sy]; //TODO
		closestNode.heuristic = getHeuristicCost(mover, sx, sy, tx, ty);//TODO
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			if(current.heuristic <= closestNode.heuristic){ //TODO
				closestNode = current; //TODO
			}
			
			/*
			if(current.compareTo(closestNode) <= 0){
				closestNode = current;
			}
			*/
				...
				for (int y=-1;y<2;y++) {
					...
					if (isValidLocation(mover, sx,sy,xp,yp)) {
						...
						if (!inOpenList(neighbour) && !(inClosedList(neighbour))) {
							...
							neighbour.heuristic = getHeuristicCost(mover, neighbour.x, neighbour.y, tx, ty);
		...
		// we've run out of search 

		if (nodes[tx][ty].parent == null) {
			
			if(closestNode.x == sx && closestNode.y == sy){
				return null;	
			}
			
			//Change target to our "closest found node"
			tx = closestNode.x;
			ty = closestNode.y;
		}
	...
	public float getHeuristicCost(Mover mover, int x, int y, int tx, int ty) {
		return (float) Math.sqrt((tx-x)*(tx-x) + (ty-y)*(ty-y));
