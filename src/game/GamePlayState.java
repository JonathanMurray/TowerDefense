...
//	private static float[] graphicsScales = new float[] {0.125f, 0.25f, 0.375f, 0.4375f,
//			0.5f, 0.5625f, 0.625f, 0.6875f, 0.75f, 0.8125f, 0.875f, 0.9375f, 1f };
//	private static int scaleIndex = 6;
//	private static final int GRAPHICS_X_TRANSLATE = (int) (((float) 1280 - (MapEditorData.PIXEL_WIDTH * graphicsScales[scaleIndex])) / 2);
//	private static final int GRAPHICS_Y_TRANSLATE = 60;
	...
	private void setup(GameContainer container){
		...
		HUD_keyChars keyChars = new HUD_keyChars(
				PlayerInputHandler.getStatChar(HeroStat.STRENGTH),
				PlayerInputHandler.getStatChar(HeroStat.DEXTERITY),
				PlayerInputHandler.getStatChar(HeroStat.INTELLIGENCE),
				PlayerInputHandler.getAbilityChars(), PlayerInputHandler.getItemChars());
		hud = new HUD(container, heroType, keyChars, Player.INSTANCE.getMaxLife(), HeroInfo.MAX_ABILITIES, HeroInfo.MAX_USABLE_ITEMS);
...
//		HeroInfo.INSTANCE.replaceAbilityWithNew(AbilityType.PUNCH, AbilityType.LIGHTNING);
//		HeroInfo.INSTANCE.replaceAbilityWithNew(AbilityType.RUN, AbilityType.SPRINT);
...
//		HeroInfo.INSTANCE.replaceAbilityWithNew(AbilityType.RUN, AbilityType.BREATH);
...
////
////		 HeroInfo.replaceAbilityWithNew(AbilityType.PUNCH, AbilityType.FURY);
...
//		g.scale(graphicsScales[scaleIndex], graphicsScales[scaleIndex]);
//		container.getInput().setOffset(-GRAPHICS_X_TRANSLATE,
//				-GRAPHICS_Y_TRANSLATE);
//		g.translate(GRAPHICS_X_TRANSLATE, GRAPHICS_Y_TRANSLATE);
		...
		hud.receiveWavesData(waves.getWaveIndex(),
				...
				waves.currentWaveHasBeenCleared(), waves.wavesHaveBegun(), waves.isFinalWave());
...
//				int i = 0;
//				for(Buff b : e.buffs){
//					g.drawString(b.toString(), e.getPixelLocation().x , e.getPixelLocation().y - 40 + i);
//					i += 15;
//				}
//			}
//		}
...
//	}
	...
	private void setupMerchant() {
		Point[] merchantLocations = new Point[] { new Point(27, 17),
				new Point(28, 17), new Point(29, 17), new Point(27, 18),
				new Point(28, 18), new Point(29, 18), new Point(27, 19),
				new Point(28, 19), new Point(29, 19) };
		merchant = new NeutralUnit("Merchant", 3000, merchantLocations,
				ResourceLoader.createNumberEncodedDirectionSpriteSet(true,
						"characters/blacksmith/blacksmith", 3, "png"), 1, 1050,
				merchantLocations[0]);
	...
	public static boolean isHeroAliveAndCloseEnoughToMerchant() {
		return HeroInfo.INSTANCE.isHeroAlive()
				&& merchant.isLocationWithinDistance(HeroInfo.INSTANCE.getHero()
						.getLocation(), 3);
	}
...
//	public static void offerHeroNewItem(ItemType newItem) { //F�RL�GG TILL HUD
